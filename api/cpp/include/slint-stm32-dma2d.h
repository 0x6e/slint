...
struct StmDma2dWindowAdapter : public slint::platform::WindowAdapter
{
    ...
    explicit StmDma2dWindowAdapter(slint::PhysicalSize size) : m_size(size) { }
...
struct StmDma2dSlintPlatform : public slint::platform::Platform
{
    using Pixel = slint::platform::Rgb565Pixel;

    static __IO bool screen_ready;

    StmDma2dWindowAdapter *m_window = nullptr;
    const slint::PhysicalSize size;
    slint::platform::SoftwareRenderer::RenderingRotation rotation;
    std::span<Pixel> buffer1;
    std::span<Pixel> buffer2;

    StmDma2dSlintPlatform(slint::PhysicalSize size,
                     slint::platform::SoftwareRenderer::RenderingRotation rotation,
    ...
    {
        auto w = std::make_unique<StmDma2dWindowAdapter>(size);
...
inline __IO bool StmDma2dSlintPlatform::screen_ready = true;

} // namespace slint::private_api

inline void slint_stm32_dma2d_init(const SlintPlatformConfiguration &config)
{
    auto a = config.size.width * config.size.height;
    std::span<slint::private_api::StmDma2dSlintPlatform::Pixel> buffer1(
            reinterpret_cast<slint::private_api::StmDma2dSlintPlatform::Pixel *>(
                    config.lcd_layer_0_address),
            a);
    std::span<slint::private_api::StmDma2dSlintPlatform::Pixel> buffer2(
            reinterpret_cast<slint::private_api::StmDma2dSlintPlatform::Pixel *>(
                    config.lcd_layer_1_address),
            a);

    slint::platform::set_platform(std::make_unique<slint::private_api::StmDma2dSlintPlatform>(
